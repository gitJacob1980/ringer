...
- 2026-09-04 code-feature/code-fix (rfr-intune pilot-status-check buckets, effort high, 3 attempts across 2 runs, ~265k tokens total): logic was correct on the first attempt; the retry (fed a 'check timed out after 60s' message) over-reached — added a 12-runspace parallel Graph pool to beat the clock, which drew HTTP 429s live within seconds. Round-2 spec that named the new data source and banned parallelism produced a clean, correct change first try (verified live, 24 s). Both FAILs were orchestrator faults: Ringer's hard 60 s CHECK_TIMEOUT_S vs a live multi-minute check, then a git-status collision with a concurrent run on other files. Lessons: (1) never hand a worker a timeout failure without saying what may/may not change to fix it — it will optimise the wrong thing; (2) budget the live check to <60 s or verify by hand; (3) concurrent runs on one repo need every other lane's files in --allowed-status. Sandbox cannot write ~/.mg so live Graph runs from the worker are impossible here — say so in the spec (done).
- 2026-08-29 code-feature (toast-alert `remediate-display-scale`, high, one task): FIRST-TRY PASS, 43k tokens, 2.5m — three new functions + integration into a 750-line PS 5.1 script against a 51-test contract; zero deviations. Tightly-specced additive work on codex-high is cheap and reliable; the spec named every function/param and pointed at a sibling implementation to copy.
- 2026-08-29 code-feature x2 (toast-alert `admin-scripts-debursttoast`, effort high, parallel, in-place repo edit): BOTH FIRST-TRY PASS — Deploy-ToastPrereqs rework 152k tokens/10m (17/17 Pester incl. linting its own generated 5.1 payload), Provision-ToastAssets rework 89k/6m (16/16). ~600-line restructures of existing scripts into library mode with removals + new hard gate; zero deviations, zero test complaints. Same orchestrator, same repo, run cleanly this time (no mid-run edits; dirty paths allow-listed up front) — confirms the earlier FAIL x2 rows were mine.
- 2026-08-29 code-feature x2 (toast-alert `production-toast-pair`, effort high, parallel, in-place repo edit): BOTH tasks were substantively FIRST-TRY PASSES — `Detect-RFRToast.ps1` 30/30 Pester + AST lint + PSSA(5.1 profile) clean, `Remediate-RFRToast.ps1` 42/42 — yet both logged FAIL x2 (183k / 210k tokens, 12m / 16m). Cause: the ORCHESTRATOR edited `docs/` and `CLAUDE.md` mid-run and the check gated on `git status` confinement, so the workers were failed for my diff and burned their retry on something they could not fix. Treat these two rows as orchestrator error, not model evidence. Model notes worth keeping: (a) both workers independently found and hardened an uncovered Intune 2048-char stdout cap on pathological input; (b) the remediate worker correctly identified two bugs in MY Pester assertions (PowerShell XML-adapter semantics), disclosed them in notes.md, and shipped the least-bad workaround rather than silently editing tests — exactly the behaviour the spec asked for. PS 5.1-targeted PowerShell written under pwsh 7 with mocked Windows edges: strong fit at high effort.
- 2026-08-29 code-feature (fallback-drain, high effort, fishers): 1/1 first-try, 136.5k tokens, 11m — the most concurrency-sensitive task of the arc (rename/restore drain of a live-appended JSONL into Postgres). Went BEYOND the spec correctly twice: added a forced read-model rebuild when draining a partially-ingested log (a duplicate case the spec only implied), and made the failure-path restore merge back a concurrently-recreated file while preserving byte offsets. High effort earns its tokens on concurrency reasoning.
